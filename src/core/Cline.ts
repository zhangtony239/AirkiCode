		...
		if (this.consecutiveMistakeCount >= 3) {
			const { response, text, images } = await this.ask(
				...
				this.api.getModel().id.includes("claude")
					...
					: "Airki Code uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 3.7 Sonnet for its advanced agentic coding capabilities.",
			)
			if (response === "messageResponse") {
				userContent.push(
					...[
